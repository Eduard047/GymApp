-- Bound the initial Friends read independently from private workout size.
-- Older released clients keep the v1 RPC. New clients page only metadata and
-- fetch a single plan through the owner-authorized detail RPC below.
...
begin
  ...
  if p_limit is null or p_limit not between 1 and 20
     ...
     or (p_cursor_created_at is not null and (
       ...
       or p_cursor_created_at < read_time - interval '31 days'
     )) then
...
create or replace function public.social_workout_invite_plan(
  p_invite_id text,
...
begin
  ...
  if p_invite_id is null or p_invite_id !~ '^wi_[0-9a-f]{32}$'
    ...
    raise exception using errcode = '22023',
      message = 'Workout invite plan request is invalid.';
  end if;
  ...
  where invite.id = p_invite_id
    ...
    and invite.revision = p_expected_revision
  ...
  if not found then
    raise exception using errcode = 'P0002',
      message = 'Social resource unavailable.';
  end if;
...
comment on function public.social_workout_inbox_page(timestamptz, text, boolean, integer)
  is 'Owner-bound metadata-only workout invite page with stable cursor and bounded rows.';
comment on function public.social_workout_invite_plan(text, bigint)
  is 'Owner-bound exact-revision plan read for one pending or recently accepted workout invite.';
