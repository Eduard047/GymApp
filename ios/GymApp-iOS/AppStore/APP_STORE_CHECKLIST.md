...
- [x] **Version/build:** current GymApp target source values are `3.0.6` (`24`). Increment the build before a replacement upload and never reuse a processed build number.
...
- [x] Production matches every repository migration. The approved 2026-08-24 rollout reached 57 migrations through `20260824180727`, including the read-only exact-session hotfix, activity-only workout sidecar, and remaining boundary hardening. Bounded owner/cross-owner/session/replay/anonymous smokes passed, authenticated PostgREST state/profile reads returned HTTP 200, and all fixtures were removed. Physical-client release checks remain separate open gates below. Earlier bounded-state evidence remains documented in [PRODUCTION_BACKEND_VERIFICATION.md](PRODUCTION_BACKEND_VERIFICATION.md).
...
- [ ] Production `garmin-sync` version 12 is `ACTIVE` by read-only metadata. The database deployment is complete; repeat OPTIONS, malformed/oversized input denial, owner-bound idempotent device creation/replay, valid-device fetch/ack/replay, token rotation, activity-only sidecar synchronization, and post-cutover continuity checks.
