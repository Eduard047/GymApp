...
- Production backend: active. A read-only 2026-08-24 readback found 54 production migrations through `20260823162119`, including friends, live rooms, Realtime invalidation, provider-neutral notifications, dispatcher scheduling, and exact Auth-session binding. Local forward migrations `20260824120000_sync_activity_only_workouts.sql` and `20260824123000_harden_remaining_supabase_boundaries.sql` are not deployed. Deploy them, read them back, and repeat final Auth, Garmin, deletion, live-room, and physical-device checks before submission.
...
- Production migration history currently stops two reviewed forward migrations behind the repository; every non-quarantined cloud state must retain revision-bound progression and bounded social-activity projections during that deployment.
...
- [ ] Production is verified through `20260823162119`, with `social-live-gateway` version 6 and `push-dispatch` version 5 active by read-only metadata. Deploy/read back the two local forward migrations and repeat focused contract/security checks. APNs/FCM/Web Push credentials and dispatcher/monitor schedules are configured; the successful empty-queue production smoke was against historical `push-dispatch` version 4 and sent no customer notifications.
