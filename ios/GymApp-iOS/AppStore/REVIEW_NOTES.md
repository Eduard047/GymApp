...
- Production backend: active. All 36 repository migrations through `20260810092029` are deployed, including friends, live rooms, Realtime invalidation, provider-neutral notifications, dispatcher scheduling, exact Auth-session binding, and the forward dispatcher-token validation fix. Earlier production verification covered social RLS/grants, revision-bound projections, revoked-session and wrong-member denial, and a rolled-back three-user friend/workout-invitation flow. Repeat final Auth, Garmin, deletion, live-room, and physical-device checks immediately before submission.
...
- Production migration history matches the repository through `20260810092029`; every non-quarantined cloud state must retain revision-bound progression and bounded social-activity projections.
- Production `social-live-gateway` version 3 and `push-dispatch` version 4 are `ACTIVE`. APNs, FCM, Web Push, the dedicated dispatcher authorization, and the dispatcher/monitor schedules are configured outside all clients. Production scheduler calls returned HTTP 200 on 2026-08-10 with zero registered installations or queued deliveries. This proves deployment, authorization, and an empty-queue path only; real APNs receipt, revocation/account-switch fencing, and notification taps remain unverified on a signed physical iPhone.
