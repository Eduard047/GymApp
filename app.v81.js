...
function render() {
  ...
  app.innerHTML = `
    ...
    <main class="screen screen-${escapeAttr(current.name)}" data-scroll-key="${escapeAttr(routeScrollKey(current))}">${socialWorkoutInviteBanner()}${pendingSharedWorkoutCard()}${screenMarkup(current)}</main>
...
function workoutShareSheetMarkup() {
  ...
  const friendOptions = cloudMode
    ? socialState.status === "loading"
      ...
      : friends.length
        ? `<div class="social-share-friends">${friends.map(friend => `<button class="leaderboard-row social-friend-row" data-action="send-workout-invite" data-profile-id="${escapeAttr(friend.profileId)}"><div>${svg("person", "small-icon")}</div><div><strong>${escapeHtml(friend.displayName)}</strong><small>${tx("Send private invitation", "Надіслати особисте запрошення")}</small></div>${svg("send", "small-icon")}</button>`).join("")}</div>`
  ...
  return `<div class="workout-share-sheet"><span class="eyebrow">${tx("Share workout", "Поділитися тренуванням")}</span><h2>${tx("Choose how to send the plan", "Обери спосіб надсилання плану")}</h2><p class="muted">${tx("Both options share only exercises and planned sets. Notes, dates, accounts, and health data stay private.", "Обидва варіанти передають лише вправи й заплановані підходи. Нотатки, дати, акаунти та health-дані залишаються приватними.")}</p><button class="button secondary full" data-action="share-workout-link">${svg("share", "small-icon")}${tx("Share a link", "Поділитися посиланням")}</button><div class="section-title"><div><span class="eyebrow">${tx("Friends", "Друзі")}</span><h3>${tx("Send inside GymApp", "Надіслати в GymApp")}</h3></div><button class="button ghost mini" data-action="refresh-social">${tx("Refresh", "Оновити")}</button></div>${friendOptions}<p class="muted">${tx("A friend accepts the plan as an editable draft. Workout sets are not synchronized live.", "Друг приймає план як редаговану чернетку. Підходи тренування не синхронізуються наживо.")}</p></div>`;
}
...
window.addEventListener("focus", () => {
  if (activeAccount?.remote !== "supabase" || document.visibilityState === "hidden" ||
      Date.now() - socialLastLoadedAt < SOCIAL_REFRESH_MIN_INTERVAL_MS ||
      socialState.status === "loading" || socialMutationInProgress) return;
