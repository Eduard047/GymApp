...
function exerciseRestTimerTarget(value) {
  ...
  if (!session || !session.sets.some(set => set.exerciseName === parsed.exerciseName)) return null;
  return parsed;
}
  ...
  try {
    ...
    for (const entry of parsed.entries) {
      ...
      if (entry.deadlineMillis <= now) {
        discardedStaleEntry = true;
        continue;
      }
      timers[normalizedKey.key] = entry.deadlineMillis;
    }
    if (discardedStaleEntry) persistExerciseRestTimers(timers, account, now);
    return { owner: descriptor.owner, timers };
  } catch {
    try {
      localStorage.removeItem(descriptor.storageKey);
    } catch {
      // The malformed value stays isolated and will be rejected on the next read.
    }
    return empty;
  }
}
...
function currentExerciseRestTimers(now = Date.now()) {
  ...
  if (changed && persistExerciseRestTimers(filtered, activeAccount, now)) {
...
function stopExerciseRestTimer(key, now = Date.now()) {
  ...
  if (!descriptor) return false;
  const next = Object.assign(Object.create(null), currentExerciseRestTimers(now));
  if (!Object.hasOwn(next, target.key)) return true;
...
async function reconcileLoadedRemoteState(cloudState, cachedState, cachedStateExists = true) {
  ...
  try {
    if (cloudState.exists) {
      ...
      const portableMergeChanged = remoteStateFingerprint(remoteState, userId) !== remoteFingerprint;
      catalogChanged = ensureBuiltInExerciseCatalog(remoteState) || portableMergeChanged;
    }
...
function addWorkoutScreen() {
  ...
  return `<section class="hero-panel add-workout-hero">
    ...
    <section class="panel note-panel"><div class="section-title"><div><span class="eyebrow">${tx("Workout date", "Дата тренування")}</span><h2>${t("note")}</h2></div><span class="pill">${escapeHtml(fmtDate(draft.startedAt))}</span></div><textarea data-draft="note" maxlength="2000" aria-label="${tAttr("note")}" placeholder="${txAttr("Push day, pull day, deload...", "Push день, pull день, делoad...")}">${escapeHtml(draft.note)}</textarea><span class="field-caption">${tx("Plan templates", "Шаблони плану")}</span><div class="chip-row">${noteTemplates().map(note => `<button class="chip buttonlike" data-action="note-template" data-note="${escapeAttr(note.value)}">${escapeHtml(note.label)}</button>`).join("")}</div></section>
    ...
    ${smartCoachPanel()}
    ${exerciseFilterControls(filteredLibraryExercises().length)}
    <section class="exercise-builder-heading"><div><span class="eyebrow">${t("addWorkout")}</span><h2>${tx("Exercises in this workout", "Вправи в цьому тренуванні")}</h2><p>${tx("Choose movements and log the working sets you plan to complete.", "Обери вправи та запиши робочі підходи, які плануєш виконати.")}</p></div><button class="button exercise-builder-add" data-action="add-block" aria-label="${tAttr("addExercise")}">${svg("add", "small-icon")}</button></section>
    <section class="draft-list">${draft.blocks.map((block, index) => draftBlock(block, index)).join("")}</section>
    <section class="panel highlighted"><div class="section-title"><div><span class="eyebrow">${tx("Workout template", "Шаблон тренування")}</span><h2>${tx("Share with a friend", "Поділитися з другом")}</h2><p>${tx("The link contains only exercises, weights, and reps. Your account, notes, and health data stay private.", "Посилання містить лише вправи, вагу й повтори. Акаунт, нотатки та дані здоров’я залишаються приватними.")}</p></div>${svg("copy", "small-icon")}</div><button class="button ghost full" data-action="share-draft">${svg("copy", "small-icon")}${tx("Share editable workout", "Поділитися редагованим тренуванням")}</button></section>
    <section class="panel watch-plan-panel"><div><span class="eyebrow">${t("addWorkout")}</span><h2>${t("syncWatch")}</h2><p class="muted">${tx("Sync sends these targets to the watch. Saving as completed records every row in history and opens the summary.", "Синхронізація надсилає ці цілі на годинник. Збереження як виконаного записує кожен рядок в історію й відкриває підсумок.")}</p></div><button class="button ghost full" data-action="sync-watch">${t("syncWatch")}</button></section>
    <button class="button full save-workout-button" data-action="save-workout">${svg("save", "small-icon")}${t("saveCompletedWorkout")}</button>`;
}
...
function smartCoachPanel() {
  return `<section class="panel highlighted smart-coach-panel"><div class="section-title"><div><span class="eyebrow">${t("smartCoach")}</span><h2>${t("generateSmart")}</h2><p>${tx("Smart Coach uses your profile and saved history to build a practical starting plan.", "Розумний тренер використовує твій профіль та збережену історію, щоб скласти практичний стартовий план.")}</p></div>${svg("auto", "small-icon")}</div><button class="button full" data-action="generate-smart">${svg("auto", "small-icon")}${t("generateSmart")}</button></section>`;
}
...
function draftBlock(block, blockIndex) {
  ...
  return `<section class="draft-exercise panel highlighted"><details open><summary class="detail-summary"><div class="draft-exercise-title"><h2>${escapeHtml(title)}</h2><p class="muted">${escapeHtml(draftSetSummary(block))}</p></div>${block.exerciseName ? exerciseMediaThumbnail(block, { blockIndex }) : ""}${block.exerciseName ? exerciseDetailBodyMap(block, "collapsed") : ""}<button class="icon-button" data-action="remove-block" data-block="${blockIndex}" aria-label="${txAttr("Remove exercise", "Прибрати вправу")}">${svg("delete")}</button></summary>
    <label>${tx("Exercise", "Вправа")}<select class="exercise-select" data-block="${blockIndex}" data-field="exerciseName"><option value="">${tx("Select exercise", "Обери вправу")}</option>${draftExerciseOptions(block)}</select></label>
...
function draftExerciseOptions(block) {
  const options = filteredLibraryExercises();
...
function smartSnapAllowedWeight(weight, movement, allowedWeightsKg, direction) {
  ...
  const candidate = increasing
    ? allowedWeightsKg.find(value => value > current)
    : [...allowedWeightsKg].reverse().find(value => value < current);
  return candidate ?? (increasing ? allowedWeightsKg.at(-1) : allowedWeightsKg[0]);
}
...
function smartRecommendation(exercise) {
  ...
  if (!sessions.length) {
    ...
    return {
      ...
      sets: Array.from({ length: smartTargetSetCount(analysis) }, () => ({
        weight: null,
        reps: smartDefaultTargetReps(analysis, repRange)
      })),
      confidence: 0.35,
      estimatedVolume: 0,
  ...
  const targetSetCount = smartTargetSetCount(analysis);
  ...
  const harderWeight = smartAdjustedWeight(latest.averageWeight, "harder", analysis, loadProfile);
  const harderWeightAvailable = loadDirection === "lowerIsHarder"
    ? harderWeight < latest.averageWeight
    : harderWeight > latest.averageWeight;
  ...
  const latestStrained = latest.minReps < repRange.min || (!isAssistanceDirection && volumeRatio < 0.88);
  ...
  const baselineSets = smartBaselineSets(latest.sets, targetSetCount, repRange);
  ...
  const sets = baselineSets.map(set => {
    ...
    if (kindId === "Deload") {
      return {
        weight: smartAdjustedWeight(
          weight,
          "easier",
          analysis,
          loadProfile,
          loadMode === "Assistance" ? null : weight * (isFatLossDeficit ? 0.9 : 0.92)
        ),
    ...
    if (kindId === "Comeback") {
      return {
        weight: smartAdjustedWeight(
          ...
          analysis,
          loadProfile,
          loadMode === "Assistance" ? null : weight * comebackMultiplier(daysSinceLastSession)
        ),
        reps: clamp(reps, repRange.min, repRange.max)
      };
    }
  ...
  if (daysSinceLastSession >= 10) reasons.push(tx(`${daysSinceLastSession} days since this exercise, so the difficulty is adjusted for the return.`, `${daysSinceLastSession} днів без цієї вправи, тому складність скориговано для повернення.`));
...
function smartTargetSetCount(analysis) {
...
function smartSessionRegressed(current, previous, loadDirection = "higherIsHarder") {
  ...
  if (loadDirection === "lowerIsHarder") {
    return current.averageWeight > previous.averageWeight + 0.01 &&
...
function detailScreen(id) {
  ...
  return `${garmin ? garminWorkoutHeader(session, garmin, grouped) : workoutHeader(session)}
    <section class="panel highlighted"><div class="row-head"><div><h2>${tx("Train together", "Тренуйтеся разом")}</h2><p class="muted">${tx("Send an editable copy without account, notes, heart rate, calories, or Garmin data.", "Надішли редаговану копію без акаунта, нотаток, пульсу, калорій і даних Garmin.")}</p></div><button class="button ghost" data-action="share-session" data-id="${escapeAttr(session.id)}">${svg("copy", "small-icon")}${tx("Share", "Поділитися")}</button></div></section>
    ${garmin ? garminWorkoutMetricsCard(garmin, session.sets.length) : ""}
...
function garminWorkoutHeader(session, metrics, grouped) {
  ...
  const completion = `${setCount} ${tx("sets", "підходів")}`;
  return `<section class="hero-panel garmin-header"><div class="row-head"><div><h2>${tx("Garmin strength workout", "Силове тренування Garmin")}</h2><p>${escapeHtml(fmtDate(session.startedAt))} · ${tx("synced from Garmin", "синхронізовано з Garmin")}</p></div><button class="icon-button hero-icon-button" data-action="delete-session" data-id="${escapeAttr(session.id)}" aria-label="${txAttr("Delete workout", "Видалити тренування")}">${svg("delete")}</button></div>
    <div class="metric-grid"><div><span>${tx("Duration", "Тривалість")}</span><strong>${escapeHtml(metrics.duration || "—")}</strong><small>${tx("watch session", "тренування з годинника")}</small></div><div><span>${tx("Logged", "Записано")}</span><strong>${escapeHtml(completion)}</strong><small>${grouped.length} ${tx("exercises", "вправ")}</small></div></div>
    <p>${tx("Synced sets are grouped below. Expand an exercise to edit weight, reps, add a missed set, or delete a wrong one.", "Синхронізовані підходи згруповано нижче. Розгорни вправу, щоб змінити вагу чи повтори, додати пропущений підхід або видалити помилковий.")}</p>
  </section>`;
}

function garminWorkoutMetricsCard(metrics, _actualSetCount) {
  const intervalSets = Array.isArray(metrics.sets)
    ? metrics.sets.filter(item => item?.interval && Number.isInteger(item.index)).slice(0, 60)
    : [];
  ...
  const intervalSection = intervalSets.length || omittedSetIntervalCount !== null
    ? `<div class="garmin-set-intervals"><h3>${tx("Chronological watch set intervals", "Хронологічні інтервали підходів годинника")}</h3><p class="muted">${escapeHtml(intervalOrderNote)}</p><p class="muted">${escapeHtml(noteProvenance)}</p>${intervalSets.map(item => {
        const interval = item.interval;
        const zones = interval.zoneSeconds
          .map((seconds, zone) => seconds > 0 ? `Z${zone} ${seconds}s` : "")
          .filter(Boolean)
          .join(" · ") || tx("No timed HR zone", "Немає виміряної зони пульсу");
        const calories = interval.garminCalories == null
          ? `Gym ${interval.gymCalories} kcal`
          : `Gym ${interval.gymCalories} kcal · Garmin ${interval.garminCalories} kcal`;
        const label = `${tx("Watch", "Годинник")} S${item.index} · ${interval.startSeconds}–${interval.endSeconds}s · ${calories} · ${zones}`;
...
function progressScreen() {
  ...
  if (!selected) return `${monthSwitcher()}<section class="panel progress-selector"><label>${tx("Exercise", "Вправа")}</label><button class="button ghost full" type="button">${tx("Select exercise", "Обери вправу")}</button></section>
...
function missionSummary(template, cadence) {
  ...
  if (cadence === "daily") {
    ...
    if (template.family === "exercises") {
      return tx(`Log ${goal} exercise entries today.`, `Занеси ${goal} вправ сьогодні.`);
    }
    if (template.family === "sets") {
      return tx(`Reach ${goal} total sets today.`, `Набери ${goal} підходів за день.`);
    }
    ...
    if (template.family === "max-session-exercises") {
      return tx(`Fit ${goal} exercises into one session today.`, `Збери ${goal} вправ в одній сесії сьогодні.`);
    }
    if (template.family === "max-session-sets") {
      return tx(`Build one session to ${goal} sets today.`, `Збери одну сесію до ${goal} підходів сьогодні.`);
    }
  }
  if (cadence === "weekly") {
    if (template.family === "workouts") {
      return tx(`Complete ${goal} workouts this week.`, `Заверши ${goal} тренувань цього тижня.`);
    }
    if (template.family === "active-days") {
      return tx(`Train on ${goal} separate days this week.`, `Потренуйся у ${goal} різні дні цього тижня.`);
    }
    if (template.family === "sets") {
      return tx(`Reach ${goal} sets this week.`, `Набери ${goal} підходів цього тижня.`);
    }
    if (template.family === "volume") {
      return tx(`Reach ${goal} total volume this week.`, `Набери ${goal} обсягу цього тижня.`);
    }
    if (template.family === "exercises") {
      return tx(`Log ${goal} exercise entries this week.`, `Занеси ${goal} вправ цього тижня.`);
    }
    if (template.family === "days-10-sets") {
      return tx(`Hit 10 sets on ${goal} different days this week.`, `Зроби 10 підходів у ${goal} дні цього тижня.`);
    }
    if (template.family === "days-1000-volume") {
      return tx(`Reach 1,000 volume on ${goal} days this week.`, `Набери 1 000 обсягу у ${goal} дні цього тижня.`);
    }
    if (template.family === "sessions-8-sets") {
      return tx(`Finish ${goal} sessions with eight or more sets this week.`, `Заверши ${goal} сесій з вісьмома або більше підходами цього тижня.`);
    }
    if (template.family === "sessions-3-exercises") {
      return tx(`Finish ${goal} sessions with three or more exercises this week.`, `Заверши ${goal} сесій з трьома або більше вправами цього тижня.`);
    }
  }
  if (cadence === "monthly") {
    if (template.family === "workouts") {
      return tx(`Complete ${goal} workouts this month.`, `Заверши ${goal} тренувань цього місяця.`);
    }
    if (template.family === "active-days") {
      return tx(`Train on ${goal} separate days this month.`, `Потренуйся у ${goal} різні дні цього місяця.`);
    }
    if (template.family === "sets") {
      return tx(`Reach ${goal} sets this month.`, `Набери ${goal} підходів цього місяця.`);
    }
    if (template.family === "volume") {
      return tx(`Reach ${goal} total volume this month.`, `Набери ${goal} обсягу цього місяця.`);
    }
    if (template.family === "exercises") {
      return tx(`Log ${goal} exercise entries this month.`, `Занеси ${goal} вправ цього місяця.`);
    }
    if (template.family === "days-10-sets") {
      return tx(`Hit 10 sets on ${goal} days this month.`, `Зроби 10 підходів у ${goal} дні цього місяця.`);
    }
    if (template.family === "days-1000-volume") {
      return tx(`Reach 1,000 volume on ${goal} days this month.`, `Набери 1 000 обсягу у ${goal} дні цього місяця.`);
    }
    if (template.family === "sessions-8-sets") {
      return tx(`Finish ${goal} sessions with eight or more sets this month.`, `Заверши ${goal} сесій з вісьмома або більше підходами цього місяця.`);
    }
    if (template.family === "sessions-3-exercises") {
      return tx(`Finish ${goal} sessions with three or more exercises this month.`, `Заверши ${goal} сесій з трьома або більше вправами цього місяця.`);
    }
    if (template.family === "max-session-volume") {
      return tx(`Push one session to ${goal} volume this month.`, `Доведи одну сесію до ${goal} обсягу цього місяця.`);
    }
    if (template.family === "max-session-sets") {
      return tx(`Build one session to ${goal} sets this month.`, `Збери одну сесію до ${goal} підходів цього місяця.`);
    }
    if (template.family === "max-session-exercises") {
      return tx(`Fit ${goal} exercises into one session this month.`, `Збери ${goal} вправ в одній сесії цього місяця.`);
    }
  }
...
function handleAction(action, el) {
  ...
  if (action === "timer") {
    if (!startExerciseRestTimer(el.dataset.key, Number(el.dataset.seconds))) {
      showToast(tx("Rest timer could not be saved.", "Не вдалося зберегти таймер відпочинку."));
    }
    return render();
  }
...
function selectBalancedSmartExercises(candidates, focus, targetMuscles, targetExerciseCount, history, variant, weeklyEffectiveSets = smartWeeklyEffectiveSets(history), weeklyTarget = smartWeeklyMuscleTarget()) {
  ...
  if (focus === "Upper") {
    const preferred = smartVariantPatterns(focus, variant);
    const press = new Set([...preferred].filter(pattern => pattern.endsWith("Press")));
    const pull = new Set([...preferred].filter(pattern => pattern.endsWith("Pull")));
    takeBestPattern(press.size ? press : new Set(["HorizontalPress", "VerticalPress"]));
    takeBestPattern(pull.size ? pull : new Set(["HorizontalPull", "VerticalPull"]));
...
function detailAddSet(sessionId, name) {
  ...
  const set = { id: uid(), exerciseName: name, ...(catalogKey ? { catalogKey } : {}), weight: last?.weight || 0, reps: last?.reps || 8, orderIndex: session.sets.filter(s => s.exerciseName === name).length };
  const setIndex = session.sets.length;
  const timerKey = `${sessionId}:${name}`;
  session.sets.push(set);
  try {
    saveState();
  } catch {
    session.sets.splice(setIndex, 1);
    showToast(tx("Could not log the set. Rest was not started.", "Не вдалося записати підхід. Таймер відпочинку не запущено."));
    return;
  }
  if (!startExerciseRestTimer(timerKey, 90)) {
    showToast(tx(
...
function startTimerTicker() {
  ...
  if (Object.values(currentExerciseRestTimers()).some(target => target > Date.now())) {
