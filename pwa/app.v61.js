...
function smartRecommendation(exercise) {
  ...
  const earnedProgression = latest.sets.length > 0 &&
    ...
    previous.sets.every(set => smartBoundedReps(set.reps, repRange.min) >= repRange.max);
...
function smartTargetSetCount(analysis) {
  ...
  return smartIsCompound(analysis) && !highFrequency && !recoveryLimited ? 4 : 3;
}
...
function smartTargetExerciseCount(focus) {
  const days = Number(state.profile.days);
  let target = focus === "FullBody"
    ? (days === 2 ? 6 : days === 3 ? 5 : 4)
    : (days <= 3 ? 6 : 5);
  if (state.profile.goal === "Strength") target -= 1;
  if (state.profile.goal === "Muscle Gain" && state.profile.calories === "Surplus") target += 1;
