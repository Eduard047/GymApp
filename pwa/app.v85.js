...
const MAX_SOCIAL_FRIEND_CODE_INPUT_CHARACTERS = 64;
const MAX_SOCIAL_FRIEND_CODE_INPUT_BYTES = 128;
const MAX_PENDING_SOCIAL_WORKOUT_REQUESTS = 25;
const MAX_PENDING_LIVE_REQUESTS = 25;
const MAX_LIVE_LOCAL_WORKOUT_ID = 2_147_483_647;
...
const SOCIAL_SHORT_FRIEND_CODE_PATTERN = /^g_[0-9a-f]{12}$/;
const SOCIAL_HUMAN_FRIEND_CODE_PATTERN = /^gym-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{4})$/i;
...
let profileHubSection = "training";
let socialState = {
  status: "idle", source: null, dashboard: null, inbox: null, friendCode: null, error: ""
};
...
const liveWorkoutAutoAttachAttempts = new Set();
...
function builtInExerciseDisplayName(builtIn, language = state.language) {
  if (builtIn.key === "straight_arm_pulldown" && language === "uk") {
    return "Журавель — тяга прямими руками";
  }
...
function exerciseDisplayName(value, language = state.language) {
  ...
  return builtInExerciseDisplayName(builtIn, language);
}
...
function exerciseSearchCandidateEntries(value, language = state.language) {
  ...
  const localizedName = builtInExerciseDisplayName(builtIn, language);
...
const russianNounForms = Object.freeze({
  ...
  "confirmed friend": ["подтверждённый друг", "подтверждённых друга", "подтверждённых друзей"],
...
function parseBoundedPostgrestError(text) {
  if (typeof text !== "string" || text.length === 0 ||
      text.length > MAX_REMOTE_ERROR_RESPONSE_BYTES ||
      new TextEncoder().encode(text).byteLength > MAX_REMOTE_ERROR_RESPONSE_BYTES) return null;
  let value;
  try {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const allowedKeys = new Set(["code", "details", "hint", "message"]);
  const keys = Object.keys(value);
  if (!Object.hasOwn(value, "code") || keys.length > allowedKeys.size ||
      keys.some(key => !allowedKeys.has(key)) ||
      typeof value.code !== "string" ||
      !/^(?:PGRST[0-9]{3}|[0-9A-Z]{5})$/.test(value.code)) return null;
  for (const field of ["details", "hint", "message"]) {
    if (!Object.hasOwn(value, field) || value[field] === null) continue;
    if (typeof value[field] !== "string" ||
        value[field].length > MAX_REMOTE_ERROR_RESPONSE_BYTES ||
        new TextEncoder().encode(value[field]).byteLength > MAX_REMOTE_ERROR_RESPONSE_BYTES) return null;
  }
...
async function supabaseRequest(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    const postgrestError = parseBoundedPostgrestError(text);
    const error = new Error(text || `Request failed: ${response.status}`);
    error.status = response.status;
    if (postgrestError) error.postgrestCode = postgrestError.code;
    throw error;
  }
...
function loadGarminPendingRevocations() {
  ...
  const now = Date.now();
  const legacyExpectedKeys = new Set([
    "version", "userId", "deviceId", "cleanupKind", "createdAt"
  ]);
  const correlatedExpectedKeys = new Set([
    ...legacyExpectedKeys, "creationRequestId"
  ]);
  for (const [userId, value] of Object.entries(parsed)) {
    const keys = value && typeof value === "object" && !Array.isArray(value)
      ? Object.keys(value)
      : [];
    const hasCreationRequestId = Object.hasOwn(value || {}, "creationRequestId");
    const expectedKeys = hasCreationRequestId
      ? correlatedExpectedKeys
      : legacyExpectedKeys;
    const createdAt = Number(value?.createdAt);
    if (!UUID_PATTERN.test(userId) || value?.version !== 1 || value.userId !== userId ||
        !UUID_V4_PATTERN.test(value.deviceId || "") ||
        (hasCreationRequestId && !UUID_V4_PATTERN.test(value.creationRequestId || "")) ||
    ...
    safe[userId] = {
      ...
      createdAt,
      ...(hasCreationRequestId
        ? { creationRequestId: value.creationRequestId.toLowerCase() }
        : {})
    };
  }
  return safe;
...
function rememberGarminPendingRevocation(
  userId,
  deviceId,
  cleanupKind = "revoke",
  ...
  if (!UUID_PATTERN.test(userId || "") || !UUID_V4_PATTERN.test(deviceId || "") ||
      ...
      (creationRequestId !== null && !UUID_V4_PATTERN.test(creationRequestId || ""))) {
    throw new Error("Garmin revocation recovery record is invalid.");
  }
  const normalizedDeviceId = deviceId.toLowerCase();
  const normalizedCreationRequestId = creationRequestId?.toLowerCase() || null;
  ...
  if (existing && (existing.deviceId !== normalizedDeviceId ||
      ...
      (normalizedCreationRequestId !== null &&
       existing.creationRequestId !== undefined &&
       existing.creationRequestId !== normalizedCreationRequestId))) {
  ...
  if (!existing) {
    records[userId] = {
      ...
      createdAt: Date.now(),
      ...(normalizedCreationRequestId !== null
        ? { creationRequestId: normalizedCreationRequestId }
        : {})
    };
    saveGarminPendingRevocations(records);
  } else if (normalizedCreationRequestId !== null &&
      existing.creationRequestId === undefined) {
    // Upgrade a same-identity legacy marker without invalidating records
    // written by already-deployed PWA versions.
    records[userId] = {
      ...existing,
      creationRequestId: normalizedCreationRequestId
    };
...
function removeGarminCreateRequestMatchingCleanup(
  userId,
  deviceId,
  cleanupKind = null,
  creationRequestId = null
  ...
  if (creationRequestId !== null) {
    if (creation.requestId !== creationRequestId.toLowerCase()) {
      throw new Error("Garmin cleanup does not match the pending creation request.");
    }
  } else {
    // Backward compatibility for pending markers written before request-ID
    // correlation existed. Their device and cleanup kind remain authoritative.
...
async function ensureGarminDeviceBinding(session) {
  ...
  if (pendingCleanup?.cleanupKind === "legacy-recovery") {
    ...
    if (!recoveryDevices.length) {
      ...
      removeGarminCreateRequestMatchingCleanup(
        ...
        pendingCleanup.creationRequestId ?? null
      );
  ...
  if (current?.recoveryPending) {
    ...
    if (!pendingDevice) {
      ...
      if (pendingAfterAbsence?.deviceId === current.deviceId) {
        removeGarminCreateRequestMatchingCleanup(
          userId,
          pendingAfterAbsence.deviceId,
          pendingAfterAbsence.cleanupKind,
          pendingAfterAbsence.creationRequestId ?? null
        );
  ...
  if (pendingCreationAfterEmptyList?.legacyFallbackAttempted) {
    ...
    removeGarminCreateRequestMatchingCleanup(
      ...
      "legacy-recovery",
      pendingCreationAfterEmptyList.requestId
    );
  }
  ...
  const binding = { version: GARMIN_CAPABILITY_VERSION, userId, deviceId: device.id };
  try {
    // The legacy endpoint chooses its own server device UUID. Correlate the
    // cleanup marker with the original request so a failed raw-record delete
    // can still be completed after restart without trusting either UUID alone.
    rememberGarminPendingRevocation(
      userId,
      device.id,
      "revoke",
...
function workoutsScreen() {
  ...
  return `
    ...
    <div class="workouts-scroll">
      ...
      <section class="workout-list">${sessions.length ? sessions.map(workoutItem).join("") : `<section class="panel highlighted empty-state-panel"><h3>${t("noWorkouts")}</h3><p>${tx("Track consistency, output and intensity at a glance.", "Відстежуй стабільність, обсяг і інтенсивність одним поглядом.")}</p><button class="button" data-action="open-add">${svg("add", "small-icon")}${tx("Start workout", "Почати тренування")}</button></section>`}</section>
    </div>
  `;
}
...
async function shareWorkoutPlan(plan, preferredIntent = null) {
  let url;
  const preferred = preferredIntent ? preferredFriendShareIntent(preferredIntent) : null;
  if (preferredIntent && !preferred) {
    modal = null;
    render();
    return showToast(tx(
      "Open the friend profile again before sharing this workout.",
      "Знову відкрий профіль друга перед поширенням цього тренування."
    ));
  }
  try {
  ...
  modal = {
    ...
    url,
    ...(preferred ? {
      preferredProfileId: preferred.preferredProfileId,
      preferredMode: preferred.preferredMode,
      expectedEpoch: preferred.expectedEpoch,
      expectedUserId: preferred.expectedUserId
    } : {})
  };
  render();
    ...
    if (pendingGarminCleanup?.cleanupKind === "legacy-recovery") {
      ...
      try {
        ...
        removeGarminCreateRequestMatchingCleanup(
          ...
          pendingGarminCleanup.creationRequestId ?? null
        );
...
function socialShortFriendCode(value) {
  if (typeof value !== "string" || !SOCIAL_SHORT_FRIEND_CODE_PATTERN.test(value)) {
    throw new TypeError("Social friend code is invalid.");
  }
  return value;
}

function normalizeSocialFriendCodeInput(value) {
  if (typeof value !== "string") return null;
  if (value.length > MAX_SOCIAL_FRIEND_CODE_INPUT_CHARACTERS ||
      [...value].length > MAX_SOCIAL_FRIEND_CODE_INPUT_CHARACTERS ||
      new TextEncoder().encode(value).byteLength > MAX_SOCIAL_FRIEND_CODE_INPUT_BYTES) return null;
  const normalized = value.trim().toLowerCase();
  if (SOCIAL_PROFILE_ID_PATTERN.test(normalized) ||
      SOCIAL_SHORT_FRIEND_CODE_PATTERN.test(normalized)) return normalized;
  const human = SOCIAL_HUMAN_FRIEND_CODE_PATTERN.exec(normalized);
  return human ? `g_${human.slice(1).join("")}` : null;
}

function formatSocialFriendCode(value) {
  if (!SOCIAL_SHORT_FRIEND_CODE_PATTERN.test(value || "")) return value || "";
  const compact = value.slice(2).toUpperCase();
  return `GYM-${compact.slice(0, 4)}-${compact.slice(4, 8)}-${compact.slice(8, 12)}`;
}

function parseSocialMyFriendCode(value) {
  const row = socialExactObject(value, ["version", "friendCode"]);
  if (row.version !== 1) throw new TypeError("Social friend code response is invalid.");
  return socialShortFriendCode(row.friendCode);
}

function isMissingSocialMyFriendCodeRpc(error) {
  return error?.status === 404 &&
    (error?.postgrestCode === "PGRST202" || error?.postgrestCode === "42883");
}
...
const SOCIAL_RPC_NAMES = new Set([
  "social_dashboard", "social_my_friend_code", "social_friend_details", "social_send_friend_request",
...
async function refreshSocialData(force = false) {
  ...
  if (!cloudMode) {
    ...
    socialRequestController = null;
    socialState = {
      status: "loaded", source, dashboard: null, inbox: null, friendCode: null, error: ""
    };
    return render();
  }
  ...
  try {
    ...
    const shortFriendCodePromise = socialRpc(
      "social_my_friend_code",
      {},
      { session, signal: socialRequestController.signal }
    ).then(parseSocialMyFriendCode).catch(error => {
      if (isMissingSocialMyFriendCodeRpc(error)) return null;
      throw error;
    });
    const [dashboardValue, inboxValue, shortFriendCode] = await Promise.all([
...
async function autoAttachStartedLiveWorkout(snapshot) {
  if (!snapshot || snapshot.room?.status !== "active") return false;
  const identity = liveSessionIdentity();
  const self = snapshot.participants?.find(participant => participant.isSelf);
  if (!identity || activeAccount?.remote !== "supabase" ||
      activeAccount.userId !== identity.userId || self?.state !== "joined" ||
      !self.progress) return false;
  if (liveWorkoutBinding?.roomId === snapshot.room.roomId) {
    return ensureLiveWorkoutAttached(snapshot);
  }
  // Joining is explicit consent to open the frozen plan when the host starts.
  // Do not steal an unrelated local workout, and do not repeat a declined draft
  // replacement prompt on every poll/realtime refresh.
  if (activeWorkout) return false;
  const attemptKey = `${liveSourceKey()}:${snapshot.room.roomId}`;
  if (liveWorkoutAutoAttachAttempts.has(attemptKey)) return false;
  if (liveWorkoutAutoAttachAttempts.size >= MAX_PENDING_LIVE_REQUESTS) {
    liveWorkoutAutoAttachAttempts.delete(liveWorkoutAutoAttachAttempts.values().next().value);
  }
  liveWorkoutAutoAttachAttempts.add(attemptKey);
  const expectedEpoch = accountEpoch;
  const attached = await ensureLiveWorkoutAttached(snapshot, true);
  return liveIdentityIsCurrent(expectedEpoch, identity) && attached;
}
...
async function refreshLiveWorkoutData(force = false, requestedRoomId = currentLiveRoomId()) {
  ...
  liveWorkoutRequestController = requestController;
  liveWorkoutState = { ...liveWorkoutState, status: "loading", source, error: "" };
  render();
  try {
    const session = loadRemoteSession();
    const fetchSnapshot = roomId => liveGateway(
      "live_snapshot",
      { roomId },
      window.GymLiveWorkout.snapshot,
      { session, signal: requestController.signal }
    ).catch(error => {
    ...
    const snapshotPromise = requestedRoomId && window.GymLiveWorkout.patterns.ROOM_ID.test(requestedRoomId)
      ? fetchSnapshot(requestedRoomId)
      : Promise.resolve(null);
    const [inbox, requestedSnapshot] = await Promise.all([inboxPromise, snapshotPromise]);
    ...
    if (!snapshot && !requestedRoomId) {
      const activeRooms = inbox.rooms.filter(room =>
        room.status === "active" && room.memberState === "joined"
      );
      // The backend normally allows one open room per participant. If that
      // invariant is ever violated, do not pick a room arbitrarily.
      if (activeRooms.length === 1) {
        snapshot = await fetchSnapshot(activeRooms[0].roomId);
        if (requestId !== liveWorkoutRequestId ||
            !liveIdentityIsCurrent(expectedEpoch, identity)) return false;
      }
    }
    ...
    if (snapshot?.room?.status === "active") {
      await autoAttachStartedLiveWorkout(snapshot);
      if (requestId !== liveWorkoutRequestId ||
          !liveIdentityIsCurrent(expectedEpoch, identity)) return false;
...
function socialRequestCards() {
  ...
  return `<section class="social-request-list"><h2>${tx("Friend requests", "Запити в друзі")}</h2><p class="muted">${tx("Accepting lets this person see only the categories allowed in Friends privacy. You can change them, remove the friend, or block them at any time.", "Після прийняття ця людина бачитиме лише категорії, дозволені в приватності Друзів. Їх можна змінити, а друга — видалити чи заблокувати будь-коли.")}</p>${incoming}${outgoing}</section>`;
}
...
function friendsPanel() {
  ...
  if (!cloudMode) {
    return `<section class="panel highlighted empty-state-panel friends-sign-in-card"><span class="eyebrow">${tx("Friends", "Друзі")}</span><h2>${tx("Cloud sign-in required", "Потрібен вхід у хмарний акаунт")}</h2><p>${tx("Friend requests, private friend progress, and workout invitations are available only in a signed-in cloud account.", "Запити в друзі, приватний прогрес друзів і запрошення на тренування доступні лише після входу в хмарний акаунт.")}</p><button class="button" data-action="profile-cloud-sign-in">${svg("person", "small-icon")}${tx("Sign in to connect", "Увійти й додати друзів")}</button></section>`;
  }
  ...
  const code = formatSocialFriendCode(socialState.friendCode || dashboard.self.friendCode);
  ...
  const urgent = [liveWorkoutInboxMarkup(), socialWorkoutInviteRows(), socialRequestCards()]
    .filter(Boolean)
    .join("");
  return `${urgent ? `<section class="profile-urgent-stack" aria-label="${txAttr("Needs your attention", "Потребує твоєї уваги")}"><div class="profile-urgent-heading"><span class="profile-live-dot" aria-hidden="true"></span><div><span class="eyebrow">${tx("NOW", "ЗАРАЗ")}</span><h2>${tx("Needs your attention", "Потребує твоєї уваги")}</h2></div></div>${urgent}</section>` : ""}
    <section class="panel friends-circle-card"><div class="row-head"><div><span class="eyebrow">${tx("Your training circle", "Твоє тренувальне коло")}</span><h2>${dashboard.friends.length ? n(dashboard.friends.length, "confirmed friend", "confirmed friends", "підтверджений друг", "підтверджені друзі", "підтверджених друзів") : tx("Build your first training connection", "Створи перший тренувальний зв’язок")}</h2><p>${socialState.error ? escapeHtml(socialState.error) : tx("Only confirmed friends appear here. Open a profile to share a workout or start live together.", "Тут є лише підтверджені друзі. Відкрий профіль, щоб надіслати тренування або запустити live разом.")}</p></div><button class="button ghost mini" data-action="refresh-social" ${loading ? "disabled" : ""}>${loading ? tx("Loading", "Завантаження") : tx("Refresh", "Оновити")}</button></div><div class="friends-circle-metrics"><span><strong>${dashboard.self.xp ?? "—"}</strong> XP</span><span><strong>${dashboard.self.level ?? "—"}</strong> ${tx("level", "рівень")}</span><span><strong>${dashboard.self.workouts ?? "—"}</strong> ${tx("workouts", "тренувань")}</span></div></section>
    <section class="leaderboard-list friend-circle-list" aria-label="${txAttr("Confirmed friends", "Підтверджені друзі")}">${rows.length ? rows.map(friendRankingRow).join("") : `<div class="empty">${tx("No shared friend progress yet.", "Спільного прогресу друзів ще немає.")}</div>`}</section>
    <section class="panel highlighted social-code-card"><div class="section-title"><div><span class="eyebrow">${tx("Add friends", "Додати друзів")}</span><h2>${tx("Connect by friend code", "З’єднатися за кодом друга")}</h2><p>${tx("Share your random code or enter a friend's. Nothing becomes visible until the request is accepted.", "Поділися випадковим кодом або введи код друга. Нічого не стане видимим, доки запит не буде прийнято.")}</p></div></div><div class="field-row"><input id="social-own-code" aria-label="${txAttr("Your friend code", "Твій код друга")}" readonly value="${escapeAttr(code)}"><div class="actions"><button class="button ghost" data-action="copy-friend-code">${tx("Copy", "Копіювати")}</button><button class="button secondary" data-action="share-friend-code">${tx("Share", "Поділитися")}</button></div></div><div class="field-row"><input id="social-friend-code" aria-label="${txAttr("Friend code", "Код друга")}" maxlength="34" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="GYM-A1B2-C3D4-E5F6"><button class="button" data-action="send-friend-request" ${socialMutationInProgress ? "disabled" : ""}>${tx("Send request", "Надіслати запит")}</button></div></section>
    <details class="panel social-privacy-card"><summary><span><span class="eyebrow">${tx("Privacy", "Приватність")}</span><strong>${tx("What confirmed friends can see", "Що бачать підтверджені друзі")}</strong></span></summary><div class="social-privacy-controls"><label class="toggle-row"><input id="social-allow-requests" type="checkbox" ${privacy.allowRequests ? "checked" : ""}><span>${tx("Allow new friend requests", "Дозволити нові запити в друзі")}</span></label><label class="toggle-row"><input id="social-share-progress" type="checkbox" ${privacy.shareProgress ? "checked" : ""}><span>${tx("XP, level, and workout count", "XP, рівень і кількість тренувань")}</span></label><label class="toggle-row"><input id="social-share-workouts" type="checkbox" ${privacy.shareRecentWorkouts ? "checked" : ""}><span>${tx("Five recent workout summaries", "П’ять останніх підсумків тренувань")}</span></label><label class="toggle-row"><input id="social-share-records" type="checkbox" ${privacy.shareRecords ? "checked" : ""}><span>${tx("Recorded exercise bests", "Записані найкращі результати у вправах")}</span></label><button class="button ghost full" data-action="save-social-privacy" data-revision="${dashboard.self.settingsRevision}" ${socialMutationInProgress ? "disabled" : ""}>${tx("Save visibility", "Зберегти видимість")}</button></div></details>
...
function friendsProfileScreen() {
  const dashboard = socialState.dashboard;
  const friendCount = dashboard?.friends.length || 0;
  const pendingCount = (dashboard?.incoming.length || 0) +
...
function currentShareableFriendCode() {
  const raw = socialState.friendCode || socialState.dashboard?.self.friendCode;
  return normalizeSocialFriendCodeInput(raw) ? formatSocialFriendCode(raw) : null;
}

async function copyCurrentFriendCode() {
  const code = currentShareableFriendCode();
  if (!code || !navigator.clipboard?.writeText) {
    showToast(tx("Clipboard access is unavailable.", "Доступ до буфера обміну недоступний."));
    return false;
  }
  try {
    await navigator.clipboard.writeText(code);
    showToast(tx("Friend code copied.", "Код друга скопійовано."));
    return true;
  } catch {
...
async function shareCurrentFriendCode() {
  const code = currentShareableFriendCode();
  if (!code) return false;
  if (typeof navigator.share !== "function") return copyCurrentFriendCode();
  try {
    await navigator.share({ title: tx("Friend code", "Код друга"), text: code });
    return true;
  } catch (error) {
    if (error?.name === "AbortError") return false;
    return copyCurrentFriendCode();
  }
}

async function sendFriendRequest() {
  const input = app.querySelector("#social-friend-code");
  const friendCode = normalizeSocialFriendCodeInput(input?.value || "");
  if (!friendCode) {
...
function liveWorkoutExerciseMatrixMarkup(snapshot) {
  const exercises = Array.isArray(snapshot?.plan?.exercises) ? snapshot.plan.exercises : [];
  const participants = Array.isArray(snapshot?.participants) ? snapshot.participants : [];
  const self = participants.find(participant => participant?.isSelf === true);
  const peer = participants.find(participant => participant?.isSelf === false);
  if (!exercises.length || !self || !peer) return "";
  const completedIds = participant => new Set(
    Array.isArray(participant?.progress?.completedSets)
      ? participant.progress.completedSets.map(set => set.setId)
      : []
  );
  const laneMarkup = (participant, exercise) => {
    const laneName = participant.isSelf
      ? tx("You", "Ти")
      : participant.profile?.displayName || tx("Friend", "Друг");
    const completed = completedIds(participant);
    const chips = exercise.sets.map((set, index) => {
      const done = completed.has(set.setId);
      const setNumber = index + 1;
      const label = `${tx("Set", "Підхід")} ${setNumber}: ${done
        ? tx("Recorded", "Записано")
        : tx("Planned", "Заплановано")}`;
      return `<span class="chip ${done ? "selected" : "muted"}" data-live-set-state="${done ? "completed" : "pending"}" aria-label="${escapeAttr(label)}">${setNumber}${done ? " ✓" : ""}</span>`;
    }).join("");
    return `<div class="row-line" role="row" data-live-lane="${participant.isSelf ? "self" : "peer"}"><strong role="rowheader">${escapeHtml(laneName)}</strong><div class="actions" role="cell" aria-label="${escapeAttr(`${tx("Progress", "Прогрес")}: ${laneName}`)}">${chips}</div></div>`;
  };
  const rows = exercises.map(exercise => {
    const displayName = exerciseDisplayName(exercise);
    return `<article class="panel"><div class="row-head"><div><strong>${escapeHtml(displayName)}</strong><small>${exercise.sets.length} ${tx("sets", "підходів")}</small></div></div><div role="table" aria-label="${escapeAttr(`${tx("Progress", "Прогрес")}: ${displayName}`)}">${laneMarkup(self, exercise)}${laneMarkup(peer, exercise)}</div></article>`;
  }).join("");
  return `<section aria-labelledby="live-exercise-progress-title"><div class="section-title"><div><span class="eyebrow">${tx("Friends progress", "Прогрес друзів")}</span><h3 id="live-exercise-progress-title">${tx("You", "Ти")} · ${escapeHtml(peer.profile?.displayName || tx("Friend", "Друг"))}</h3><p class="muted">${tx("Progress is synchronizing. Open the room to see both lanes.", "Прогрес синхронізується. Відкрий кімнату, щоб бачити обох учасників.")}</p></div></div>${rows}</section>`;
}
...
function friendDetailMarkup() {
  ...
  return `<div class="friend-detail-sheet"><span class="eyebrow">${tx("Friend profile", "Профіль друга")}</span><h2>${escapeHtml(detail.friend.displayName)}</h2><p class="muted">${tx("Synced data entered by your friend; not independently verified.", "Синхронізовані дані, внесені другом; вони не перевіряються незалежно.")}</p>${progress}<section class="friend-training-card"><span class="eyebrow">${tx("NEXT SET", "НАСТУПНИЙ ПІДХІД")}</span><h3>${tx("Choose a saved workout for both of you", "Обери збережене тренування для вас обох")}</h3><p>${tx("Live synchronizes both set lanes. Sending a workout creates an independent editable copy.", "Live синхронізує обидві шкали підходів. Надсилання тренування створює незалежну редаговану копію.")}</p><div class="friend-training-actions"><button class="button full" data-action="open-friend-workout-picker" data-share-mode="live" data-profile-id="${escapeAttr(profileId)}">${svg("fitness", "small-icon")}${tx("Train together", "Тренуватися разом")}</button><button class="button secondary full" data-action="open-friend-workout-picker" data-share-mode="copy" data-profile-id="${escapeAttr(profileId)}">${svg("copy", "small-icon")}${tx("Send workout", "Надіслати тренування")}</button></div></section><h3>${tx("Recent workouts", "Останні тренування")}</h3>${workouts}<h3>${tx("Recorded exercise bests", "Записані найкращі результати")}</h3>${records}<details class="friend-safety-actions"><summary>${tx("Friend controls", "Керування другом")}</summary><div class="actions vertical"><button class="button ghost full" data-action="remove-friend" data-friendship-id="${escapeAttr(relation.friendshipId)}" data-revision="${relation.friendshipRevision}">${tx("Remove friend", "Видалити друга")}</button><button class="button danger full" data-action="block-friend" data-profile-id="${escapeAttr(profileId)}">${tx("Block profile", "Заблокувати профіль")}</button></div></details></div>`;
}

function preferredFriendShareIntent(value = modal) {
  if (!["friend-workout-picker", "workout-share"].includes(value?.type) ||
      value.expectedEpoch !== accountEpoch ||
      !UUID_PATTERN.test(value.expectedUserId || "") ||
      activeAccount?.remote !== "supabase" ||
      activeAccount.userId !== value.expectedUserId ||
      loadRemoteSession()?.user?.id !== value.expectedUserId ||
      !SOCIAL_PROFILE_ID_PATTERN.test(value.preferredProfileId || "") ||
      !["copy", "live"].includes(value.preferredMode) ||
      !socialState.dashboard?.friends.some(friend => friend.profileId === value.preferredProfileId)) {
    return null;
  }
  return {
    type: value.type,
    preferredProfileId: value.preferredProfileId,
...
function openFriendWorkoutPicker(element) {
  const profileId = element.dataset.profileId;
  const preferredMode = element.dataset.shareMode;
  const expectedUserId = activeAccount?.userId;
  if (modal?.type !== "friend-detail" || modal.profileId !== profileId ||
      !["copy", "live"].includes(preferredMode) ||
      !SOCIAL_PROFILE_ID_PATTERN.test(profileId || "") ||
      !UUID_PATTERN.test(expectedUserId || "") ||
      activeAccount?.remote !== "supabase" ||
      loadRemoteSession()?.user?.id !== expectedUserId ||
      !socialState.dashboard?.friends.some(friend => friend.profileId === profileId)) return false;
  modal = {
    type: "friend-workout-picker",
...
function friendWorkoutPickerMarkup() {
  const intent = preferredFriendShareIntent(modal);
  const friend = intent
    ? socialState.dashboard?.friends.find(row => row.profileId === intent.preferredProfileId)
    : null;
  if (!intent || !friend) {
    return `<div class="friend-workout-picker"><h2>${tx("Workout picker expired", "Вибір тренування застарів")}</h2><p class="muted">${tx("Open the friend profile again before choosing a workout.", "Знову відкрий профіль друга перед вибором тренування.")}</p></div>`;
  }
  const sessions = [...state.sessions]
    .filter(session => Number.isSafeInteger(session.id) && session.id > 0 && Array.isArray(session.sets) && session.sets.length)
    .sort((left, right) => right.startedAt - left.startedAt || right.id - left.id)
    .slice(0, 12);
  const actionLabel = intent.preferredMode === "live"
    ? tx("Continue to live", "Перейти до live")
    : tx("Continue to send", "Перейти до надсилання");
  const rows = sessions.map(session => {
    const summary = sessionSummary(session);
    return `<button class="friend-workout-choice" data-action="choose-friend-workout" data-id="${escapeAttr(session.id)}"><span><strong>${escapeHtml(fmtDate(session.startedAt))}</strong><small>${summary.exercises} ${tx("exercises", "вправ")} · ${summary.sets} ${tx("sets", "підходів")}</small></span><b>${escapeHtml(actionLabel)}</b></button>`;
  }).join("");
  return `<div class="friend-workout-picker"><span class="eyebrow">${intent.preferredMode === "live" ? "LIVE" : tx("SEND A COPY", "НАДІСЛАТИ КОПІЮ")}</span><h2>${tx("Choose a saved workout for", "Обери збережене тренування для")} ${escapeHtml(friend.displayName)}</h2><p class="muted">${tx("Nothing is sent yet. Review the sharing method on the next screen.", "Поки нічого не надіслано. Перевір спосіб поширення на наступному екрані.")}</p><div class="friend-workout-picker-list">${rows || `<div class="empty"><strong>${tx("No completed workout to share yet", "Ще немає завершеного тренування для поширення")}</strong><p>${tx("Save a workout first, then return to this friend profile.", "Спочатку збережи тренування, а потім повернися до профілю друга.")}</p></div>`}</div>${state.sessions.length > sessions.length ? `<p class="muted">${tx("Showing the 12 most recent workouts.", "Показано 12 останніх тренувань.")}</p>` : ""}</div>`;
}

function chooseFriendWorkout(element) {
  const intent = preferredFriendShareIntent(modal);
  const sessionId = Number(element.dataset.id);
  const session = Number.isSafeInteger(sessionId) && sessionId > 0
    ? state.sessions.find(row => row.id === sessionId)
    : null;
  if (!intent || !session) return false;
  try {
    return shareWorkoutPlan(sharedWorkoutPlanFromSession(session), intent);
  } catch {
...
function workoutShareSheetMarkup() {
  const preferred = preferredFriendShareIntent(modal);
  const friends = [...(socialState.dashboard?.friends || [])].sort((left, right) => {
    if (left.profileId === preferred?.preferredProfileId) return -1;
    if (right.profileId === preferred?.preferredProfileId) return 1;
    return left.displayName.localeCompare(right.displayName, displayLocale());
  });
  ...
  const friendOptions = cloudMode
    ? socialState.status === "loading"
      ...
      : friends.length
        ? `<div class="social-share-friends">${friends.map(friend => {
            const isPreferred = friend.profileId === preferred?.preferredProfileId;
            const liveFirst = isPreferred && preferred.preferredMode === "live";
            const copyButton = `<button class="button ${isPreferred && !liveFirst ? "" : "ghost"} mini" data-action="send-workout-invite" data-profile-id="${escapeAttr(friend.profileId)}">${svg("copy", "small-icon")}${tx("Send copy", "Надіслати копію")}</button>`;
            const liveButton = `<button class="button ${liveFirst ? "" : "ghost"} mini" data-action="send-live-workout-invite" data-profile-id="${escapeAttr(friend.profileId)}">${svg("fitness", "small-icon")}LIVE</button>`;
            return `<article class="panel social-request-card social-share-friend-card ${isPreferred ? "preferred" : ""}"><div>${isPreferred ? `<span class="eyebrow">${tx("SELECTED FRIEND", "ОБРАНИЙ ДРУГ")}</span>` : ""}<strong>${escapeHtml(friend.displayName)}</strong><p class="muted">${tx("Choose an independent copy or a synchronized room.", "Обери незалежну копію або синхронізовану кімнату.")}</p></div><div class="actions">${liveFirst ? `${liveButton}${copyButton}` : `${copyButton}${liveButton}`}</div></article>`;
          }).join("")}</div>`
...
function spotlight(exercise, grouped, allTimeBest) {
  ...
  if (!points.length) {
    return `<h2>${escapeHtml(displayName)}</h2><p>${escapeHtml(tx(`Log sets for ${displayName} to unlock trends.`, `Додай підходи для ${displayName}, щоб відкрити тренди.`))}</p><button class="button hero-outline" data-action="open-add">${svg("add", "small-icon")}${tx("Start workout", "Почати тренування")}</button>`;
  }
...
function bindEvents() {
  ...
  app.querySelectorAll('.profile-hub-switch [role="tab"][data-action="profile-hub-section"]').forEach(el => {
    el.addEventListener("keydown", activateProfileHubTabFromKeyboard);
  });
...
function activateProfileHubTabFromKeyboard(event) {
  ...
  const tabs = [...tablist.querySelectorAll('[role="tab"][data-action="profile-hub-section"]')]
    .filter(tab => tab.dataset.section === "training" || tab.dataset.section === "settings");
  ...
  if (typeof nextTab?.click !== "function") return false;
  const nextSection = nextTab.dataset.section === "settings" ? "settings" : "training";
  event.preventDefault?.();
  nextTab.click();
  requestAnimationFrame(() => {
    app.querySelector(
      `.profile-hub-switch [role="tab"][data-action="profile-hub-section"][data-section="${nextSection}"][aria-selected="true"]`
...
async function handleAction(action, el) {
  ...
  if (action === "profile-hub-section") {
    if (route().name !== "leaderboard") return false;
    profileHubSection = el.dataset.section === "settings" ? "settings" : "training";
    return render();
  }
  if (action === "profile-cloud-sign-in") {
    if (route().name !== "leaderboard") return false;
    profileHubSection = "settings";
    render();
  ...
  if (action === "copy-friend-code") return copyCurrentFriendCode();
  if (action === "share-friend-code") return shareCurrentFriendCode();
...
function activeWorkoutEntityId(usedIds, maximum = Number.MAX_SAFE_INTEGER) {
  if (!Number.isSafeInteger(maximum) || maximum < 1) {
    throw new Error("Active workout ID range is invalid.");
  }
  for (let attempt = 0; attempt < 64; attempt += 1) {
    const generated = uid();
    const candidate = maximum === Number.MAX_SAFE_INTEGER
      ? generated
      : generated % maximum + 1;
...
function activeWorkoutFromDraft(
  draft = workoutDraft,
  now = Date.now(),
  workoutIdMaximum = Number.MAX_SAFE_INTEGER
) {
  ...
  const id = activeWorkoutEntityId(usedIds, workoutIdMaximum);
...
async function startWorkout({ liveSnapshot = null, liveIdentity = null } = {}) {
  ...
  try {
    const liveStartRequested = liveSnapshot !== null || liveIdentity !== null;
    candidate = activeWorkoutFromDraft(
      workoutDraft,
      Date.now(),
      liveStartRequested ? MAX_LIVE_LOCAL_WORKOUT_ID : Number.MAX_SAFE_INTEGER
    );
    if (liveStartRequested) {
...
function parseAppPushData(value) {
  const row = socialExactObject(value, ["version", "target"], ["roomId"]);
  if (row.version !== 1 || !["live", "social"].includes(row.target)) {
    throw new TypeError("Push target is invalid.");
  }
  const hasRoomId = Object.hasOwn(row, "roomId");
  if (row.target === "social") {
    if (hasRoomId) throw new TypeError("Social push target cannot select a live room.");
    return { version: 1, target: "social" };
  }
  if (!hasRoomId) return { version: 1, target: "live" };
  if (!window.GymLiveWorkout?.patterns?.ROOM_ID?.test(row.roomId || "")) {
    throw new TypeError("Live push room is invalid.");
  }
  return { version: 1, target: "live", roomId: row.roomId };
}

async function openLiveWorkoutPushRoom(roomId) {
  if (!window.GymLiveWorkout?.patterns?.ROOM_ID?.test(roomId || "")) return false;
  const identity = liveSessionIdentity();
  const expectedEpoch = accountEpoch;
  if (!identity || activeAccount?.remote !== "supabase" ||
      activeAccount.userId !== identity.userId) return false;
  const refreshed = await refreshLiveWorkoutData(true, roomId);
  if (!refreshed || !liveIdentityIsCurrent(expectedEpoch, identity)) return false;
  const visible = liveWorkoutState.snapshot?.room?.roomId === roomId ||
    liveWorkoutState.inbox?.rooms?.some(room => room.roomId === roomId) ||
    liveWorkoutState.inbox?.invitations?.some(invite => invite.roomId === roomId) ||
    liveWorkoutBinding?.roomId === roomId;
  if (!visible) return false;
  if (activeWorkout && liveWorkoutBinding?.roomId === roomId &&
      liveWorkoutBinding.localWorkoutId === activeWorkout.id) {
    modal = null;
    nav = [{ name: "workouts" }, { name: "active" }];
    replaceNavigationHistory();
...
function openAppPushTarget(value) {
  ...
  if (target.target === "live") {
    if (activeAccount?.remote === "supabase") {
      if (target.roomId) void openLiveWorkoutPushRoom(target.roomId);
      else void refreshLiveWorkoutData(true);
    }
  } else {
...
function captureAppPushTargetFromLocation() {
  try {
    ...
    const kinds = url.searchParams.getAll("notification");
    if (kinds.length !== 1 || !["live", "social"].includes(kinds[0])) return null;
    const kind = kinds[0];
    const rooms = url.searchParams.getAll("room");
    let target = null;
    try {
      target = parseAppPushData({
        version: 1,
        target: kind,
        ...(rooms.length === 1 ? { roomId: rooms[0] } : {})
      });
      if (rooms.length > 1) target = null;
    } catch {
      target = null;
    }
...
if ("serviceWorker" in navigator) {
  ...
  navigator.serviceWorker.addEventListener?.("message", event => {
    ...
    try {
      socialExactObject(message, ["version", "type", "target"], ["roomId"]);
      if (message.version === 1 && message.type === "gymapp_notification_click") {
        openAppPushTarget({
          version: 1,
          target: message.target,
          ...(Object.hasOwn(message, "roomId") ? { roomId: message.roomId } : {})
        });
      }
    } catch {
