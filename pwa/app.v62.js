...
function smartRecommendation(exercise) {
  ...
  const canAssessPlateau = loadMode !== "Assistance" && latest.maxWeight > 0;
  const canAssessRegression = loadMode !== "Assistance" &&
    (latest.maxWeight > 0 || loadMode === "Bodyweight");
  ...
  const completedRepCeiling = smartCompletedAtRepCeiling(latest, targetSetCount, repRange.max) &&
    ...
    smartPerformanceDidNotDecline(latest, previous, targetSetCount);
  const needsHarderBodyweight = completedRepCeiling && loadMode === "Bodyweight" && latest.maxWeight <= 0;
  const earnedProgression = completedRepCeiling && !needsHarderBodyweight;
  ...
  const sets = baselineSets.map(set => {
    ...
    if (kindId === "ProgressiveOverload") {
      ...
      return {
        weight: weight > 0
          ? smartBoundedWeight(loadMode === "Assistance" ? Math.max(0, weight - step) : weight + step)
          : 0,
    ...
    if (kindId === "Deload") {
      return {
        weight: loadMode === "Assistance"
          ? smartBoundedWeight(weight + chooseWeightStep(weight))
          : smartBoundedWeight(weight * (isFatLossDeficit ? 0.9 : 0.92)),
    ...
    if (kindId === "Comeback") {
      return {
        weight: loadMode === "Assistance"
          ? smartBoundedWeight(weight + chooseWeightStep(weight))
          : smartBoundedWeight(weight * comebackMultiplier(daysSinceLastSession)),
        reps: clamp(reps, repRange.min, repRange.max)
      };
    }
...
function smartPerformanceDidNotDecline(latest, previous, targetSetCount) {
  ...
  return latestWeight >= previousWeight && latestReps >= previousReps;
}
...
function smartTargetExerciseCount(focus) {
  const days = Number(state.profile.days);
  let target = days === 2 ? 6 : days === 3 ? 5 : days === 4 ? 4 : 3;
  if (state.profile.goal === "Strength") target -= 1;
  if (state.profile.calories === "Deficit") target -= 1;
  const minimum = focus === "FullBody" ? 3 : 2;
  return clamp(target, minimum, 6);
}
