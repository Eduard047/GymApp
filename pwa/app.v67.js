...
function draftBlock(block, blockIndex) {
  ...
  const rec = block.exerciseName ? smartRecommendation(block, {
...
function garminWorkoutHeader(session, metrics, grouped) {
  ...
  return `<section class="hero-panel garmin-header"><div class="row-head"><div><h2>${tx("Garmin-format strength workout", "Силове тренування у форматі Garmin")}</h2><p>${escapeHtml(fmtDate(session.startedAt))} · ${tx("metrics parsed from the saved note", "показники прочитано зі збереженої нотатки")}</p></div><button class="icon-button hero-icon-button" data-action="delete-session" data-id="${escapeAttr(session.id)}" aria-label="${txAttr("Delete workout", "Видалити тренування")}">${svg("delete")}</button></div>
    <div class="metric-grid"><div><span>${tx("Duration", "Тривалість")}</span><strong>${escapeHtml(metrics.duration || "—")}</strong><small>${tx("saved session", "збережене тренування")}</small></div><div><span>${tx("Logged", "Записано")}</span><strong>${escapeHtml(completion)}</strong><small>${grouped.length} ${tx("exercises", "вправ")}</small></div></div>
    <p>${tx("Workout sets are grouped below. Expand an exercise to edit weight, reps, add a missed set, or delete a wrong one.", "Підходи тренування згруповано нижче. Розгорни вправу, щоб змінити вагу чи повтори, додати пропущений підхід або видалити помилковий.")}</p>
  </section>`;
}
...
function applySmartAlternative(blockIndex, replacementId) {
  ...
  const recommendation = smartRecommendation(replacement, {
    appliedEffort: block.smartEffort,
    hardSlot: block.smartHardSlot,
    recoverySteps: block.smartRecoverySteps
  });
  block.sets = recommendation.sets.map(set => ({ weight: set.weight ?? "", reps: set.reps }));
  modal = null;
...
function applySmart(blockIndex) {
  const block = workoutDraft?.blocks[blockIndex];
  if (!block) return;
  block.sets = smartRecommendation(block, {
    appliedEffort: block.smartEffort,
    hardSlot: block.smartHardSlot === true,
    recoverySteps: block.smartRecoverySteps
