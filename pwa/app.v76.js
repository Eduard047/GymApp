...
async function handleAction(action, el) {
  ...
  if (action === "open-workout-exercise-picker") return openWorkoutExercisePicker(
    el.dataset.pickerTarget,
    Number(el.dataset.block),
  ...
  if (action === "timer") {
    if (!await startExerciseRestTimer(el.dataset.key, Number(el.dataset.seconds))) {
...
function openWorkoutExercisePicker(target, blockIndex = null, sessionId = null) {
  ...
  if (target === "session" &&
      (!Number.isSafeInteger(sessionId) || sessionId <= 0 || !state.sessions.some(item => item.id === sessionId))) return false;
  modal = {
...
function selectWorkoutExercise(exerciseId) {
  ...
  if (picker.target === "session") return quickAddExercise(exerciseId, picker.sessionId);
  if (!workoutDraft) return false;
  const catalogKey = persistedExerciseCatalogKey(exercise);
...
function detailAddSet(sessionId, name) {
  ...
  if (!startExerciseRestTimerLocked(timerKey, 90)) {
    showToast(tx(
