...
function parseActiveWorkoutEnvelope(input, account = activeAccount) {
  const descriptor = activeWorkoutAccountDescriptor(account);
...
function loadActiveWorkoutCommitLedger(account = activeAccount) {
  ...
  try {
    ...
    const workouts = value.workouts.map((candidate, index) => {
      const workout = parseActiveWorkoutEnvelope(candidate, account);
    ...
    return { ledger: { version: 1, owner: descriptor.owner, workouts }, raw };
  } catch {
    return null;
  }
}
...
function loadActiveWorkoutRecord(account = activeAccount) {
  ...
  try {
    ...
    return { workout: parseActiveWorkoutEnvelope(raw, account), raw };
  } catch {
...
function loadStoredStateBase(account = activeAccount) {
  ...
  try {
    ...
    return raw ? validateImportedEnvelope(raw, fallback).state : fallback;
  } catch {
    return fallback;
  }
}
  ...
  try {
    ...
    const cleanupResult = descriptor
      ? await withActiveWorkoutDeletionLock(
          descriptor,
          () => purgeDeletedCloudAccountFromBrowser(account)
        )
      : { acquired: false, value: false };
    let cleanupComplete = cleanupResult.acquired && cleanupResult.value === true;
    if (!cleanupResult.acquired) {
      // The server-side account is already gone. Stop any queued mutation from
      // becoming current, leave remaining private site data explicitly
      // reported as uncleared, and close the deleted account in this tab.
      cleanupComplete = false;
      removeActiveAccountMarkerForDeletion(account);
      finishRemovedAccountTransition();
    }
...
async function deleteLocalAccount() {
  ...
  accountTransitionInProgress = true;
  try {
    const result = await withActiveWorkoutDeletionLock(activeWorkoutDescriptor, () => {
      let snapshots;
      try {
        snapshots = {
...
async function finishActiveWorkout() {
  ...
  const result = await withActiveWorkoutMutationLock(
    ...
    () => finishActiveWorkoutLocked(mutationContext)
  );
  if (!result.acquired) return activeWorkoutMutationUnavailable(mutationContext);
  return result.value === true;
}
