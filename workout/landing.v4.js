(function (root, factory) {
  ...
  function parse(rawUrl) {
    ...
    const prefix = `${codec.HASH_KEY}=`;
    if (rawHash.length > prefix.length + codec.LIMITS.encodedLength ||
        !rawHash.startsWith(prefix) || rawHash.includes("&") ||
        [...CREDENTIAL_KEYS].some(key => rawHash.toLowerCase().includes(`${key}=`)) ||
        rawHash.toLowerCase().includes("token=")) {
      fail("Shared workout URL is invalid.");
    }
