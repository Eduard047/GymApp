...
const [
  ...
  deletionSql,
  preauthSql,
  preauthWrapperFixSql,
  coalesceFixSql,
  identityIsolationSql,
  hardeningSql,
  garminBudgetSql,
  followupPgTap,
  ...
  readFile(
    "supabase/migrations/20260823160702_harden_live_sessions_and_push_ownership.sql",
    "utf8",
  ),
  readFile(
    "supabase/migrations/20260823160703_bound_workout_duration_sync.sql",
    "utf8",
  ),
  readFile(
  ...
  readFile(
    "supabase/migrations/20260823160706_add_edge_preauth_budgets.sql",
    "utf8",
  ),
  readFile(
    "supabase/migrations/20260823161839_fix_edge_preauth_service_wrapper.sql",
    "utf8",
  ),
  readFile(
    "supabase/migrations/20260823162119_fix_security_hardening_coalesce_calls.sql",
    "utf8",
  ),
  readFile(
    "supabase/migrations/20260825105114_isolate_verified_edge_rate_limits.sql",
    "utf8",
  ),
  readFile(
  ...
  readFile(
    "supabase/tests/security_scan_followup_boundaries.sql",
    "utf8",
  ),
  ...
  readFile(
    "app/src/main/java/com/example/gymapp/auth/AndroidKeystoreAuthStore.kt",
    "utf8",
  ),
]);
...
test("direct owner RLS and live gateway reject revoked or not-after sessions", () => {
  ...
  for (
    const policy of [
      ...
      "Users can read own state",
      "Users can insert own state",
    ...
    assert.match(
      sessionSql.slice(start, start + 650),
      /current_auth_session_is_live/,
    );
  }
  assert.match(
    sessionSql,
    /live_gateway_require_session[\s\S]*session\.not_after/,
  );
});
...
test("public Edge rate limits use verified identities without a shared global bucket", () => {
  ...
  assert.match(
    preauthSql,
    /grant execute on function public\.edge_preauth_debit\(text, text\)[\s\S]*to service_role/,
  );
  assert.doesNotMatch(
    preauthSql,
    /grant execute on function public\.edge_preauth_debit\(text, text\)[\s\S]*to (?:anon|authenticated)/,
  );
  assert.match(
    preauthWrapperFixSql,
  ...
  assert.doesNotMatch(
    preauthWrapperFixSql,
    /grant usage on schema gymapp_private/,
  );
  ...
  assert.match(
    identityIsolationSql,
    /source_hash = pg_catalog\.repeat\('0', 64\)/,
  );
  ...
  assert.doesNotMatch(
    socialSource,
    /preauth-budget|debitVerifiedIdentityBudget/,
  );
  ...
  assert.match(
    hardeningSql,
    /social_require_caller\(p_action text\)[\s\S]*social_live_debit_budget\(/,
  );
  ...
  assert.match(garminSource, /debitGarminGatewayBudget/);
  assert.doesNotMatch(garminSource, /debitVerifiedIdentityBudget/);
  ...
  assert.match(garminBudgetSql, /generate_series\(0, 63\)/);
  assert.match(garminBudgetSql, /\) <> 192/);
  assert.doesNotMatch(garminBudgetSql, /device_token|bearer_token/i);
});
...
test("push installation ownership transfer requires exact provider-address possession", () => {
  ...
  assert.match(
    sessionSql,
    /foreign_installation\.token_fingerprint is distinct from requested_fingerprint/,
  );
...
test("PWA rejects complex JSON, oversized routes, and image pixel bombs before expensive parsers", () => {
  assert.ok(
    stateSource.indexOf("preflightJsonText(input)") <
      stateSource.indexOf("JSON.parse(input)"),
  );
  assert.match(stateSource, /Backup contains duplicate object keys/);
  assert.ok(
    pwaSource.indexOf("customExerciseImageHeaderDimensions(file)") <
      pwaSource.indexOf("createImageBitmap(file)"),
  );
  assert.match(
    pwaSource,
    /boundedLocationParameters\(window\.location\.search, 8192\)/,
  );
  assert.match(
    pwaSource,
    /rawHash\.length > window\.GymSharedWorkout\.LIMITS\.encodedLength/,
  );
});
...
test("follow-up pgTAP fixture covers all four committed security boundaries", () => {
  assert.match(followupPgTap, /select plan\(30\)/);
  assert.match(followupPgTap, /garmin_gateway_preauth_buckets/);
  assert.match(followupPgTap, /social_my_friend_code_direct_worker/);
  assert.match(followupPgTap, /account_deletion_operations/);
  assert.match(followupPgTap, /push_authorize_delivery_send/);
  assert.match(followupPgTap, /select \* from finish\(\);\s+\nrollback;/);
});
