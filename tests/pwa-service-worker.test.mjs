...
const VERSIONED_ASSET_PAIRS = [
  ["confirmed.css", "confirmed.v45.css"],
  ["confirmed.js", "confirmed.v45.js"],
  ["frame-guard.js", "frame-guard.v45.js"],
  ["styles.css", "styles.v45.css"],
  ["muscle-regions.js", "muscle-regions.v45.js"],
  ["supabase-config.js", "supabase-config.v45.js"],
  ["state-contract.js", "state-contract.v45.js"],
  ["garmin-cloud-sync.js", "garmin-cloud-sync.v45.js"],
  ["progression-rules.js", "progression-rules.v45.js"],
  ["app.js", "app.v45.js"]
];
...
test("service worker caches only exact immutable v45 same-origin assets", () => {
  for (const scope of ["https://example.test/", "https://example.test/GymApp/"]) {
    const handler = loadWorker(scope).listeners.get("fetch");

    assert.equal(isIntercepted(handler, new URL("./app.v45.js", scope)), true);
    assert.equal(isIntercepted(handler, new URL("./state-contract.v45.js", scope)), true);
    assert.equal(isIntercepted(handler, new URL("./frame-guard.v45.js", scope)), true);
    assert.equal(isIntercepted(handler, new URL("./app.js?v=45", scope)), false);
...
test("v45 pathname assets cannot be mistaken for predecessor v44 assets", () => {
  ...
  const v45Paths = [
    "confirmed.v45.css", "confirmed.v45.js", "frame-guard.v45.js", "styles.v45.css",
    "muscle-regions.v45.js", "supabase-config.v45.js", "state-contract.v45.js",
    "garmin-cloud-sync.v45.js", "progression-rules.v45.js", "app.v45.js"
  ];

  for (const pathname of v45Paths) {
    assert.equal(predecessorPaths.has(pathname), false, pathname);
    assert.match(workerSource, new RegExp(`\\./${pathname.replaceAll(".", "\\.")}`));
  }
  assert.doesNotMatch(workerSource, /\.\/app\.js\?v=45/);
});

test("every deployed v45 asset exists and is byte-identical to its canonical source", () => {
...
test("service worker ignores credential-bearing, partial-content, and non-GET requests", () => {
  ...
  assert.equal(isIntercepted(handler, "https://example.test/GymApp/app.v45.js", {
  ...
  assert.equal(isIntercepted(handler, "https://example.test/GymApp/app.v45.js", { method: "POST" }), false);
});
...
test("token-like asset queries are neither intercepted nor cached", () => {
  const handler = loadWorker().listeners.get("fetch");
  assert.equal(isIntercepted(handler, "https://example.test/GymApp/app.v45.js?provider_token=secret"), false);
...
test("cached documents receive the same anti-framing policy", async () => {
  ...
  assert.equal(await response.text(), "cached");
  assert.deepEqual(worker.openedCaches, ["gym-pwa-v46"]);
...
test("install reloads one internally consistent version without taking over old clients", async () => {
  ...
  assert.deepEqual(worker.openedCaches, ["gym-pwa-v46"]);
  assert.ok(worker.addedAssets.some(asset => asset.url.endsWith("/app.v45.js")));
  assert.ok(worker.addedAssets.some(asset => asset.url.endsWith("/styles.v45.css")));
