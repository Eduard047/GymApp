...
const VERSIONED_ASSET_PAIRS = [
  ...
  ["russian-text.js", "russian-text.v58.js"],
  ["app.js", "app.v59.js"]
];
...
test("service worker caches only exact immutable current same-origin assets", () => {
  for (const scope of ["https://example.test/", "https://example.test/GymApp/"]) {
    ...
    assert.equal(isIntercepted(handler, new URL("./app.v59.js", scope)), true);
...
test("current versioned pathname assets cannot be mistaken for predecessor assets", () => {
  ...
  const currentPaths = [
    "confirmed.v56.css", "confirmed.v56.js", "frame-guard.v56.js", "theme.v56.js", "styles.v58.css",
    "muscle-regions.v56.js", "supabase-config.v56.js", "state-contract.v56.js",
    "garmin-cloud-sync.v56.js", "progression-rules.v56.js", "russian-text.v58.js", "app.v59.js"
  ];
...
test("service worker ignores credential-bearing, partial-content, and non-GET requests", () => {
  ...
  assert.equal(isIntercepted(handler, "https://example.test/GymApp/app.v59.js", {
  ...
  assert.equal(isIntercepted(handler, "https://example.test/GymApp/app.v59.js", { method: "POST" }), false);
});
...
test("token-like asset queries are neither intercepted nor cached", () => {
  const handler = loadWorker().listeners.get("fetch");
  assert.equal(isIntercepted(handler, "https://example.test/GymApp/app.v59.js?provider_token=secret"), false);
...
test("cached documents receive the same anti-framing policy", async () => {
  ...
  assert.equal(await response.text(), "cached");
  assert.deepEqual(worker.openedCaches, ["gym-pwa-v82"]);
...
test("install reloads one internally consistent version without taking over old clients", async () => {
  ...
  assert.deepEqual(worker.openedCaches, ["gym-pwa-v82"]);
  assert.ok(worker.addedAssets.some(asset => asset.url.endsWith("/app.v59.js")));
  assert.ok(worker.addedAssets.some(asset => asset.url.endsWith("/theme.v56.js")));
  assert.ok(worker.addedAssets.some(asset => asset.url.endsWith("/styles.v58.css")));
