...
test("audited Ukrainian runtime labels keep their intended workout, progress, auth, and Garmin meaning", () => {
  ...
  const adjacentLiteralCases = new Map([
    ...
    ["Copy this Garmin pairing token into Connect IQ settings now. Treat it as a password. Temporary retry material has been removed and the token will not be shown again. Choose Cancel to revoke it.", "Зараз скопіюй цей токен сполучення Garmin у налаштування Connect IQ. Стався до нього як до пароля. Тимчасові дані повтору вже видалені, і токен більше не буде показано. Натисни «Скасувати», щоб відкликати його."],
