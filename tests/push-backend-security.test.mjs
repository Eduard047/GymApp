...
const gatewayPath = new URL(
  "../supabase/functions/social-live-gateway/index.ts",
  import.meta.url,
);
const gatewayDenoPath = new URL(
  "../supabase/functions/social-live-gateway/deno.json",
  import.meta.url,
);
...
const providersPath = new URL(
  "../supabase/functions/push-dispatch/providers.ts",
  import.meta.url,
);
const dispatcherDenoPath = new URL(
  "../supabase/functions/push-dispatch/deno.json",
  import.meta.url,
);
const dispatcherReadmePath = new URL(
...
const sendAuthorizationMigrationPath = new URL(
...
const [
  ...
  readFile(sendAuthorizationMigrationPath, "utf8"),
...
function sqlFunction(source, qualifiedName) {
  ...
  const createStart = source.indexOf(`create function ${qualifiedName}(`);
...
test("push storage is private, RLS-enabled, account-bound, and scrubs revoked addresses", () => {
  for (
    ...
    assert.match(
      pushSql,
      new RegExp(
        `alter table gymapp_private\\.${table} enable row level security;`,
      ),
    );
    assert.match(
      pushSql,
      new RegExp(`revoke all on table gymapp_private\\.${table}`),
    );
  }
  ...
  assert.match(
    pushSql,
    /provider_token = null,[\s\S]*web_push_p256dh = null,[\s\S]*web_push_auth = null,[\s\S]*binding_id = pg_catalog\.gen_random_uuid\(\),[\s\S]*revoked_at = request_time/,
  );
  assert.match(
    pushSql,
  ...
  assert.match(
    pushSql,
    /then gymapp_private\.notification_installations\.revision[\s\S]*else gymapp_private\.notification_installations\.revision \+ 1/,
  );
  assert.match(
    pushSql,
    /set installation_revision = stored_row\.revision,[\s\S]*delivery\.status = 'pending'[\s\S]*outbox\.recipient_user_id = caller_user_id/,
  );
  assert.match(
    pushSql,
    /revoked_at < request_time - interval '7 days'[\s\S]*limit 32/,
  );
  assert.match(
    pushSql,
    /consume_notification_rate_limit\(caller_user_id, 'register'\)/,
  );
  assert.match(
    pushSql,
    /consume_notification_rate_limit\(caller_user_id, 'revoke'\)/,
  );
  ...
  assert.doesNotMatch(
    pushSql,
    /grant (?:select|insert|update|delete|all) on (?:table )?gymapp_private\.(?:notification_installations|push_outbox|push_outbox_deliveries) to/i,
  );
});

test("notification domain failures preserve the debit and legacy PostgREST error contract", () => {
  const helper = sqlFunction(
    pushSql,
    "gymapp_private.notification_domain_error_response",
  );
  ...
  const revoke = sqlFunction(
    pushSql,
    "public.notification_revoke_installation",
  );
  ...
  for (const rpc of [register, revoke]) {
    ...
    assert.ok(
      authCheck >= 0 && debit > authCheck && businessSubtransaction > debit,
    );
    assert.doesNotMatch(rpc, /when\s+others/i);
    assert.doesNotMatch(rpc, /when[^\n]*(?:42501|23[0-9A-Z]{3}|P0001)/i);
    assert.match(
      rpc,
      /get stacked diagnostics[\s\S]*domain_error_code = returned_sqlstate/,
    );
  ...
  assert.match(
    register,
    /domain_error_code = '22023'[\s\S]*Notification registration is invalid\./,
  );
  assert.match(
    register,
    /domain_error_code = '54000'[\s\S]*Notification registration cannot be updated\.[\s\S]*Notification installation limit exceeded\./,
  );
  assert.match(revoke, /when sqlstate '22023' then/);
  assert.doesNotMatch(revoke, /sqlstate '54000'/);
  assert.match(
    revoke,
    /domain_error_code is distinct from '22023'[\s\S]*domain_error_message is distinct from 'Notification revocation is invalid\.'/,
  );

  assert.match(helper, /case when p_code = '54000' then '500' else '400' end/);
  assert.match(
    helper,
    /jsonb_build_object\(\s*'code', p_code,\s*'details', nullif\(p_detail, ''\),\s*'hint', nullif\(p_hint, ''\),\s*'message', p_message/s,
  );
  assert.match(
    pushSql,
    /revoke all on function gymapp_private\.notification_domain_error_response\([\s\S]*from public, anon, authenticated, service_role;/,
  );

  assert.match(
    register,
    /'version', 1,[\s\S]*'bindingId', stored_row\.binding_id,[\s\S]*'registrationRevision', stored_row\.revision,[\s\S]*'registeredAt', stored_row\.updated_at/,
  );
  assert.match(
    revoke,
    /'version', 1,[\s\S]*'installationId', p_installation_id,[\s\S]*'revoked', revoked_count = 1/,
  );
});
...
test("durable push hooks cover lifecycle events but never enqueue set progress", () => {
  ...
  assert.doesNotMatch(
    pushSql,
    /enqueue_push_notification\([\s\S]{0,180}'(?:live_)?progress'/,
  );
  ...
  assert.match(
    pushSql,
    /delivery\.lease_token = p_lease_token[\s\S]*installation\.user_id = outbox\.recipient_user_id[\s\S]*installation\.revision = delivery\.installation_revision/,
  );
  assert.match(dispatcher, /rpc\("push_authorize_delivery_send"/);
  assert.ok(
    dispatcher.indexOf('rpc("push_authorize_delivery_send"') <
      dispatcher.indexOf("sendDelivery(delivery"),
  );
  assert.match(
    dispatcher,
    /current === "stale"[\s\S]*outcome: "permanent"[\s\S]*delivery_not_authorized/,
  );
  assert.doesNotMatch(
    dispatcher,
    /current === "stale"[\s\S]{0,240}outcome: "invalid"/,
  );
  assert.match(
    sendAuthorizationSql,
    /from auth\.sessions as session[\s\S]*for share/,
  );
  assert.match(
    sendAuthorizationSql,
    /for update of delivery, outbox, installation/,
  );
  assert.match(sendAuthorizationSql, /set send_authorized_at = coalesce/);
  assert.match(
    sendAuthorizationSql,
  ...
  assert.match(
    sendAuthorizationSql,
    /create or replace function public\.push_delivery_is_current[\s\S]*authorize_push_delivery_send/,
  );
  assert.match(
    pushSql,
    /installation\.revision = delivery_row\.installation_revision/,
  );
  assert.match(pushSql, /for update of delivery skip locked/);
  assert.match(
    pushSql,
    /completed_at < request_time - interval '30 days'[\s\S]*limit 500/,
  );
  assert.match(
    pushSql,
    /last_seen_at < request_time - interval '180 days'[\s\S]*limit 500/,
  );
  assert.match(
    pushSql,
    /revoked_at < request_time - interval '30 days'[\s\S]*limit 500/,
  );
  ...
  assert.match(
    pushSql,
    /p_invalid_registration is null[\s\S]*p_permanent_failure is null/,
  );

  const retry = sqlFunction(pushSql, "public.push_mark_retry");
  assert.match(
    retry,
    /order by \(delivery\.id = delivery_row\.id\) desc,[\s\S]*for update skip locked[\s\S]*limit 500/,
  );
  assert.match(
    retry,
    /changed_deliveries as \([\s\S]*update gymapp_private\.push_outbox_deliveries[\s\S]*returning delivery\.outbox_id[\s\S]*select distinct changed\.outbox_id[\s\S]*from changed_deliveries as changed/,
  );
  assert.doesNotMatch(
    retry,
    /select distinct delivery\.outbox_id[\s\S]*where delivery\.installation_id = delivery_row\.installation_id/,
  );
});
...
test("gateway re-verifies Auth, commits the durable perimeter debit, then validates and routes", () => {
  ...
  assert.match(
    gateway,
    /rpc\("social_gateway_perimeter_debit"[\s\S]*p_user_id: userId,[\s\S]*p_session_id: sessionId/,
  );
  assert.match(
    gateway,
    /p_user_id: userId,[\s\S]*p_session_id: sessionId,[\s\S]*p_action: parsed\.action/,
  );
  assert.match(gateway, /route\.serviceOnly\s*\? serviceClient\s*: userClient/);
  assert.match(gateway, /status === 409\s*\? "conflict"/);
  assert.match(
    perimeterSql,
    /from auth\.sessions as session[\s\S]*session\.id = p_session_id[\s\S]*session\.user_id = p_user_id[\s\S]*for key share/,
  );
  assert.match(
    perimeterSql,
    /grant execute on function public\.social_live_gateway_debit\(uuid, uuid, text\)[\s\S]*to service_role/,
  );
...
test("dispatcher is server-only, dependency-pinned, and keeps provider payload opaque", () => {
  ...
  assert.match(
    dispatcher,
    /constantTimeEqual\(dispatchServerKey, credential\)/,
  );
  ...
  assert.doesNotMatch(
    dispatcher,
    /constantTimeEqual\(suppliedDispatchServerKey, serviceKey\)/,
  );
  ...
  assert.doesNotMatch(
    dispatcher,
    /createClient\(projectUrl,\s*dispatchServerKey/,
  );
  ...
  assert.match(
    dispatcher,
    /await Promise\.all\(\[\.\.\.groups\.keys\(\)\]\.map/,
  );
  ...
  assert.match(
    providers,
    /version: 1,[\s\S]*bindingId: delivery\.binding_id,[\s\S]*kind: liveKind,[\s\S]*roomId:[\s\S]*roomRevision:/,
  );
  assert.match(
    pushSql,
    /environment text,[\s\S]*binding_id uuid,[\s\S]*provider_token text/,
  );
  assert.match(
    pushSql,
    /installation\.environment,[\s\S]*installation\.binding_id,[\s\S]*installation\.provider_token/,
  );
  assert.doesNotMatch(
    providers,
    /(?:displayName|email|weight|reps|access_token).*opaquePayload/,
  );
...
test("push dispatch has a dormant Vault-backed minute scheduler with no public execute path", () => {
  ...
  assert.match(
    schedulerSql,
    /revoke all on table gymapp_private\.push_dispatch_requests[\s\S]*from public, anon, authenticated, service_role/,
  );
  ...
  assert.match(
    dispatcherReadme,
    /gymapp_push_dispatch_server_key[\s\S]*dedicated[\s\S]*PUSH_DISPATCH_SERVER_KEY/,
  );
  ...
  assert.match(
    schedulerFixSql,
    /octet_length\(dispatch_token\) not between 43 and 256/,
  );
  ...
  assert.match(
    schedulerSql,
    /jsonb_build_object\('version', 1, 'batchSize', 10\)/,
  );
  assert.match(schedulerSql, /timeout_milliseconds := 120000/);
  assert.match(
    schedulerSql,
    /pg_advisory_xact_lock\([\s\S]*gymapp-push-dispatch-scheduler-v1/,
  );
  assert.match(
    schedulerSql,
    /tracked\.outcome = 'pending'[\s\S]*tracked\.requested_at >=[\s\S]*interval '5 minutes'[\s\S]*return null/,
  );
  assert.match(
    schedulerSql,
  ...
  assert.match(
    schedulerSql,
    /with responses as materialized[\s\S]*limit 100[\s\S]*get diagnostics resolved_count/,
  );
  assert.match(
    schedulerSql,
    /with missing as materialized[\s\S]*limit 100[\s\S]*outcome = 'missing'/,
  );
  assert.match(
    schedulerSql,
    /with expired as materialized[\s\S]*tracked\.outcome <> 'pending'[\s\S]*interval '30 days'[\s\S]*limit 500/,
  );
  assert.doesNotMatch(
    schedulerSql,
    /set[\s\S]{0,160}(?:error_msg|content|headers)\s*=/,
  );
  assert.match(
    schedulerSql,
  ...
  assert.match(
    schedulerSql,
    /job\.username = current_user[\s\S]*job\.database = current_database\(\)/,
  );
  ...
  assert.match(
    schedulerSql,
    /revoke all on function gymapp_private\.dispatch_push_notifications\(\)[\s\S]*from public, anon, authenticated, service_role/,
  );
  assert.match(
    schedulerSql,
    /revoke all on function gymapp_private\.monitor_push_dispatch_responses\(\)[\s\S]*from public, anon, authenticated, service_role/,
  );
  assert.match(
    schedulerSql,
    /has_table_privilege\([\s\S]*'service_role', 'gymapp_private\.push_dispatch_requests', 'SELECT'/,
  );
