...
function achievementsCard() {
  ...
  return `<section class="panel highlighted"><h2>${t("achievements")}</h2><p class="muted">${tx("Recent unlocks and the next solo milestones.", "Останні відкриття й наступні особисті віхи.")}</p>
    ${achievements.map(a => `<div class="achievement"><div class="percent">${Math.min(100, Math.round(a.progress / a.target * 100))}%</div><div><strong>${escapeHtml(a.title)}</strong><p>${escapeHtml(a.description)}</p><div class="progress"><span class="${percentageClass(a.progress / a.target * 100)}"></span></div><small>${Math.round(a.progress)} / ${a.target}</small></div></div>`).join("")}
  </section>`;
}
...
function smartRecommendation(name) {
  ...
  if (latestStrained) reasons.push(tx("Recent reps or volume dipped, so the plan stays conservative.", "Повтори або обсяг знизилися, тому план залишається обережним."));
  ...
  if (kindId === "ProgressiveOverload") reasons.push(tx("The increase is intentionally conservative.", "Збільшення навмисно невелике."));
